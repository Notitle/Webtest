...
class Category_DAO
{
    ...
    {
        $query = "SELECT true FROM category WHERE cat_id = :a";
        $result = $this->pdo->prepare($query);
        $result->execute(array(":a" => $cm->getId()));
        if ($result->rowCount() != 0)
        {
            $categorie = $this->pdo->prepare("UPDATE category SET cat_name=:n, cat_parent=:p WHERE cat_id=:a");
            $arr = array(':a' => $cm->getId(), ':n' => $cm->getName(), ':p' => $cm->getParentId());
        }
        else
        {
            $categorie = $this->pdo->prepare("INSERT INTO category(cat_name,cat_parent) VALUES(:n,:p)");
            $arr = array(':n' => $cm->getName(), ':p' => $cm->getParentId());
        }
        $categorie->execute($arr);
    }
